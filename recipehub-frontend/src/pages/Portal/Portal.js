  ...
  const avgRating = totalRecipes > 0
    ...
    useEffect(() => {
      const fetchTopRecipeAndRecipes = async () => {
        ...
        try {
          const res = await fetch(`http://localhost:5000/api/recipe/user/${user.id}`);
            ...
            <>
              ...
              <div className="edit-profile-form">
                ...
                <div className="edit-profile-form-input-wrapper">
                  ...
                  <br/>
                  <input value={username} disabled className="edit-form-item-input" />
                </div>
      ...
      <>
        ...
        <div className="portal-body-grid-item">
            ...
            <div className="portal-body-recipes-filters-container">
              <button
                className={`filter-newer ${filterType === 'newer' ? 'active-filter' : ''}`}
                onClick={() => setFilterType('newer')}
              >
                Newer
              </button>
              <button
                className={`filter-older ${filterType === 'older' ? 'active-filter' : ''}`}
                onClick={() => setFilterType('older')}
              >
                Older
              </button>
              <button
                className={`filter-top ${filterType === 'top' ? 'active-filter' : ''}`}
              ...
              {filteredRecipes.length === 0 ? (
                ...
                sortedRecipes.map((recipe) => (
                  <div key={recipe._id} className="portal-body-recipes-item">
                    <h1 className="portal-body-recipes-item-title">{recipe.title}</h1>
